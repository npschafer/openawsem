...
def create_single_memory(pdb, chain):
    ...
    if chain == "-1":
        ...
        logging.info(f"Chains info read from crystal_structure.pdb, chains to simulate: {chain}")
    ...
    for c in chain:
        logging.debug(f"Convert chain {c} of crystal structure to Gro file")
    ...
    with open("single_frags.mem", "w") as out:
        ...
        for (chain_name, chain_start_residue_index, seq_length) in seq_data:
            logging.debug(f"Write single memory for chain {chain_name}")

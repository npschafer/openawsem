...
atom_type = {'1' : 'C', '2' : 'N', '3' : 'O', '4' : 'C', '5' : 'H', '6' : 'C', '7': 'H'}
atom_desc = {'1' : 'C-Alpha', '2' : 'N', '3' : 'O', '4' : 'C-Beta', '5' : 'H-Beta', '6' : 'C-Prime', '7': 'H-Nitrogen'}
PDB_type = {'1' : 'CA', '2' : 'N', '3' : 'O', '4' : 'CB', '5' : 'HB', '6' : 'C', '7': 'H'}
...
def convertToPDB():
  ...
  for ia in atoms2:
    ...
    if not ires in ch_map:
      ...
      sys.exit()
    ch = ch_map[ires]
    atom = PDB_Atom(iatom, PDB_type[ia.No_m], resname, ch, ires, ia.x, ia.y, ia.z, ia.ty)
    if resname == 'PRO' and ia.desc == 'H-Nitrogen':
      continue
    if resname == 'GLY' and ia.desc == 'H-Beta':
      continue
    atoms3.append(atom)
    iatom += 1
...
def buildAllAtoms(build_terminal_atoms=True, build_bonds=False):
    ...
    for i in range(1, nres_tot+1):
        ...
        # Determine the desired order for residue i:
        order = ['N', 'H', 'CA', 'C', 'O', 'CB']

        add_atoms = []
        for role in order:
            if role == 'N' and i in N_atom_map:
                add_atoms.append(N_atom_map[i]) if i in N_atom_map else None
            elif role == 'CA':
                add_atoms.append(Ca_atom_map[i]) if i in Ca_atom_map else None
            elif role == 'C':
                add_atoms.append(Cp_atom_map[i]) if i in Cp_atom_map else None
            elif role == 'O':
                add_atoms.append(O_atom_map[i]) if i in O_atom_map else None
            elif role == 'CB':
                add_atoms.append(Cb_atom_map[i]) if i in Cb_atom_map else None
            elif role == 'H':
                add_atoms.append(H_atom_map[i]) if i in H_atom_map else None
